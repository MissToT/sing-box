...
If the effective domain resolution `strategy` (including an inherited `dns.strategy`) is `prefer_ipv4` or `prefer_ipv6`, only addresses of the preferred family are dialed concurrently at first. The other family is tried concurrently only after all preferred addresses fail, or immediately if no preferred addresses are available. `fallback_delay` does not trigger an early switch between address families in this mode. `ipv4_only` and `ipv6_only` still restrict resolution to the selected family.
