...
// prefixableList mirrors [badoption.Prefixable.UnmarshalJSON] so the tables below
// can keep plain string literals: a bare address becomes a full-length prefix.
func prefixableList(t *testing.T, values ...string) badoption.Listable[*badoption.Prefixable] {
	t.Helper()
	list := make(badoption.Listable[*badoption.Prefixable], 0, len(values))
	for _, value := range values {
		...
		list = append(list, common.Ptr(badoption.Prefixable(prefix)))
	}
	return list
}
...
func TestHeadlessRuleEntryCount(t *testing.T) {
	...
	for _, test := range []struct {
		...
		},
		{
			...
			options: option.DefaultHeadlessRule{
				SourceIPCIDR: prefixableList(t, "192.0.2.1", "2001:db8::1"),
				IPCIDR:       prefixableList(t, "198.51.100.1", "203.0.113.0/24", "2001:db8:1::/48"),
			},
			count: 5,
		},
		{
			...
			options: option.DefaultHeadlessRule{
				...
				IPCIDR:       prefixableList(t, "192.0.2.0/24", "2001:db8::/32"),
				Port:         []uint16{443},
				Network:      []string{"tcp"},
				Invert:       true,
			},
		...
		},
		{
			...
			options: option.DefaultHeadlessRule{
				IPCIDR: prefixableList(t, "0.0.0.0/0", "::/0"),
			},
			count: 2,
		},
		{
	...
	for version := uint8(1); version <= C.RuleSetVersionCurrent; version++ {
		t.Run(fmt.Sprint(version), func(t *testing.T) {
			options := option.DefaultHeadlessRule{
				...
				IPCIDR: prefixableList(t, "10.0.0.0/24", "10.0.1.0/25"),
				Port:   []uint16{443},
			}
			expected := uint64(12)
...
func TestHeadlessRuleEntryCountUsesAvailableEntries(t *testing.T) {
	...
	options := option.DefaultHeadlessRule{
		...
		IPCIDR:       prefixableList(t, "10.0.0.0/25", "10.0.0.128/25", "2001:db8::/32", "2001:db8:1::/48"),
	}
